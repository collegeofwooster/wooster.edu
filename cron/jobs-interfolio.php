...
// pull events feed
$jobs_raw = simplexml_load_file( 'https://logic.interfolio.com/byc-search/15631/feed', 'SimpleXMLElement', LIBXML_NOCDATA );
...
// debug full list of jobs
// print_r( $jobs->channel->item ); die;
...
if ( !empty( $jobs->channel->item ) ) {
	...
	foreach ( $jobs->channel->item as $job ) {
		...
		// debug first result and die for testing (prevents a full loop through all records)
		// print_r( $job ); die;
		...
		update_post_meta( $post_id, '_p_job_education', $job->qualifications );
		update_post_meta( $post_id, '_p_job_instructions', $job->application_instructions );
		update_post_meta( $post_id, '_p_job_eeo_statement', $job->eeo_statement );

		// see if the term exists
		$cat_info = term_exists( $job->maincategory, 'job_cat' );
		...
		if ( $cat_info ) {
			...
			if ( wp_set_post_terms( $post_id, $cat_info['term_id'], 'job_cat', 1 ) ) {
				echo "Added job to category: " . $job->maincategory . "\n";
			}

		} else {

			// create the category (returns either new category ID or old one)
			$cat_info = wp_insert_term( $job->maincategory, 'job_cat' );

			// if that worked
			if ( $cat_info ) {
				echo "Added job category: " . $job->maincategory . "\n";
			}
